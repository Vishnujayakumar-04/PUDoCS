// Utility script to add M.Tech Data Science & AI 1st Year students
...
/**
 * Add M.Tech Data Science & AI 1st Year students to Firestore
...
export const addMtechDSStudents = async () => {
    ...
    console.log(`📋 Total students to import: ${M_TECH_DS_1ST_YEAR_STUDENTS.length}`);
    
    const students = M_TECH_DS_1ST_YEAR_STUDENTS.map(student => {
        const formatted = formatStudentData(student.registerNumber, student.name);
 ...
 */
export const addSingleMtechDSStudent = async (registerNumber, name) => {
    const studentData = formatStudentData(registerNumber, name);
    ...
    try {
        ...
        // Also save to local storage
        const savedStudent = { id: registerNumber, ...studentData };
        await studentStorageService.addStudent(savedStudent);
...
export { M_TECH_DS_1ST_YEAR_STUDENTS };
