...
/**
 * Student Local Storage Service (Web Version)
 * Handles caching student data locally for offline access using localStorage
 */
...
export const studentStorageService = {
    ...
    saveStudents: async (students) => {
        try {
            ...
            localStorage.setItem(STUDENTS_LAST_SYNC_KEY, new Date().toISOString());
    ...
    getStudents: async () => {
        try {
            const dataStr = localStorage.getItem(STUDENTS_STORAGE_KEY);
            if (dataStr) {
    ...
    clearStudents: async () => {
        try {
            ...
            localStorage.removeItem(STUDENTS_LAST_SYNC_KEY);
    ...
    getLastSync: async () => {
        try {
            const lastSync = localStorage.getItem(STUDENTS_LAST_SYNC_KEY);
